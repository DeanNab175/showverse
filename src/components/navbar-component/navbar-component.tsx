...
function NavbarComponent() {
  ...
  useGSAP(
    () => {
      ...
      const targetY = activeLink.getBoundingClientRect().top - navRect.top;

      if (!hasInitialized.current) {
        gsap.fromTo(
          indicator,
          { y: 0, opacity: 0 },
          { y: targetY, opacity: 1, duration: 0.6, ease: "bounce.out" }
        );
  ...
  */
  ...
  return (
    <nav className="main-nav h-full grid items-center relative isolate">
      <div
        ref={navIndicatorRef}
        className="active-nav-indicator -z-10 w-12 h-12 rounded-xl absolute top-0 left-1/2 -translate-x-1/2 bg-primary opacity-0"
      ></div>
